...
export default function FlavorEditor({ flavor, steps, onReloadSteps, onReloadFlavors, notify }: Props) {
  ...
  const [flavorActive, setFlavorActive] = useState(flavor.is_active);
  ...
  const saveFlavor = async () => {
    setSaving(true);
    const { error } = await supabase.from("humor_flavors").update({ name: flavorName.trim(), description: flavorDesc.trim() || null, is_active: flavorActive }).eq("id", flavor.id);
    setSaving(false);
    ...
    <div style={{ padding: 32, maxWidth: 800 }}>
        ...
        {!editingFlavor ? (
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
            <div>
              ...
              <div style={{ marginTop: 8, display: "inline-flex", alignItems: "center", gap: 6, background: flavor.is_active ? "rgba(5,150,105,0.08)" : "var(--surface2)", border: `1px solid ${flavor.is_active ? "rgba(5,150,105,0.2)" : "var(--border)"}`, borderRadius: 100, padding: "3px 10px" }}>
                <span style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 10, color: flavor.is_active ? "var(--accent3)" : "var(--text-dim)" }}>{flavor.is_active ? "● Active" : "○ Inactive"}</span>
              </div>
            </div>
            <div style={{ display: "flex", gap: 8 }}>
              <button onClick={() => { setFlavorName(flavor.name); setFlavorDesc(flavor.description ?? ""); setFlavorActive(flavor.is_active); setEditingFlavor(true); }} style={btnS}>Edit</button>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
            ...
            <label style={{ display: "flex", alignItems: "center", gap: 8, fontFamily: "'JetBrains Mono', monospace", fontSize: 12, color: "var(--text-muted)", cursor: "pointer" }}>
              <input type="checkbox" checked={flavorActive} onChange={e => setFlavorActive(e.target.checked)} style={{ width: "auto", accentColor: "var(--accent)" }} />
              Active
            </label>
      ...
      {showAddStep && (
        <div style={{ background: "var(--surface)", border: "1.5px solid var(--accent)", borderRadius: 12, padding: 20, marginBottom: 16, boxShadow: "var(--shadow)" }}>
          ...
          <textarea style={{ ...inp, minHeight: 100, resize: "vertical" as const, fontFamily: "'JetBrains Mono', monospace", fontSize: 13 }} placeholder="Describe what this step should do. Use {input} to reference the previous step's output or the original image URL." value={newInstruction} onChange={e => setNewInstruction(e.target.value)} autoFocus />
          ...
          <div key={step.id} style={{ background: "var(--surface)", border: `1.5px solid ${editingStepId === step.id ? "var(--accent)" : "var(--border)"}`, borderRadius: 12, padding: 20, boxShadow: "var(--shadow)", opacity: moving === step.id ? 0.5 : 1, transition: "all 0.15s" }}>
            <div style={{ display: "flex", alignItems: "flex-start", gap: 16 }}>
              ...
              <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 4, flexShrink: 0 }}>
                ...
                <button onClick={() => moveStep(step.id, "up")} disabled={idx === 0 || !!moving} style={{ background: "transparent", border: "1px solid var(--border)", borderRadius: 4, padding: "2px 6px", color: "var(--text-dim)", fontSize: 10, transition: "all 0.1s" }}
                  onMouseEnter={e => e.currentTarget.style.borderColor = "var(--accent)"}
                  onMouseLeave={e => e.currentTarget.style.borderColor = "var(--border)"}>↑</button>
                <button onClick={() => moveStep(step.id, "down")} disabled={idx === sorted.length - 1 || !!moving} style={{ background: "transparent", border: "1px solid var(--border)", borderRadius: 4, padding: "2px 6px", color: "var(--text-dim)", fontSize: 10, transition: "all 0.1s" }}
                  onMouseEnter={e => e.currentTarget.style.borderColor = "var(--accent)"}
                  onMouseLeave={e => e.currentTarget.style.borderColor = "var(--border)"}>↓</button>
              </div>
